...
// OpenGL3Renderer implements a renderer based on github.com/go-gl/gl (v3.2-core).
type OpenGL3Renderer struct {
	imguiIO imgui.IO
...
// NewOpenGL3Renderer attempts to initialize a renderer.
// An OpenGL context has to be established before calling this function.
func NewOpenGL3Renderer(io imgui.IO) (*OpenGL3Renderer, error) {
	err := gl.Init()
	if err != nil {
...
func (renderer *OpenGL3Renderer) Dispose() {
	renderer.invalidateDeviceObjects()
}

// PreRender clears the framebuffer.
func (renderer *OpenGL3Renderer) PreRender(clearColor [4]float32) {
	gl.ClearColor(clearColor[0], clearColor[1], clearColor[2], clearColor[3])
	gl.Clear(gl.COLOR_BUFFER_BIT)
}

// Render translates the ImGui draw data to OpenGL3 commands.
func (renderer *OpenGL3Renderer) Render(displaySize [2]float32, framebufferSize [2]float32, drawData imgui.DrawData) {
...
func (renderer *OpenGL3Renderer) createDeviceObjects() {
	// Backup GL state
	var lastTexture int32
	var lastArrayBuffer int32
	var lastVertexArray int32
...
func (renderer *OpenGL3Renderer) createFontsTexture() {
	// Build texture atlas
	io := imgui.CurrentIO()
...
func (renderer *OpenGL3Renderer) invalidateDeviceObjects() {
