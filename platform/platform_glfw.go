...
// GLFWPlatform implements a platform based on github.com/go-gl/glfw (v3.2).
...
// NewGLFWPlatform attempts to initialize a GLFW context.
func NewGLFWPlatform(io imgui.IO, width int, height int, title string) (*GLFWPlatform, error) {
...
func (platform *GLFWPlatform) Dispose() {
	platform.window.Destroy()
	glfw.Terminate()
}

// ShouldStop returns true if the window is to be closed.
func (platform *GLFWPlatform) ShouldStop() bool {
	return platform.window.ShouldClose()
}

// ProcessEvents handles all pending window events.
func (platform *GLFWPlatform) ProcessEvents() {
	glfw.PollEvents()
}

// DisplaySize returns the dimension of the display.
func (platform *GLFWPlatform) DisplaySize() [2]float32 {
	w, h := platform.window.GetSize()
	return [2]float32{float32(w), float32(h)}
}

// FramebufferSize returns the dimension of the framebuffer.
func (platform *GLFWPlatform) FramebufferSize() [2]float32 {
...
func (platform *GLFWPlatform) NewFrame() {
...
func (platform *GLFWPlatform) PostRender() {
	platform.window.SwapBuffers()
}

func (platform *GLFWPlatform) setKeyMapping() {
...
func (platform *GLFWPlatform) installCallbacks() {
...
func (platform *GLFWPlatform) mouseButtonChange(window *glfw.Window, rawButton glfw.MouseButton, action glfw.Action, mods glfw.ModifierKey) {
...
func (platform *GLFWPlatform) mouseScrollChange(window *glfw.Window, x, y float64) {
	platform.imguiIO.AddMouseWheelDelta(float32(x), float32(y))
}

func (platform *GLFWPlatform) keyChange(window *glfw.Window, key glfw.Key, scancode int, action glfw.Action, mods glfw.ModifierKey) {
...
func (platform *GLFWPlatform) charChange(window *glfw.Window, char rune) {
	platform.imguiIO.AddInputCharacters(string(char))
}

// ClipboardText returns the current clipboard text, if available.
func (platform *GLFWPlatform) ClipboardText() (string, error) {
	return platform.window.GetClipboardString()
}

// SetClipboardText sets the text as the current clipboard text.
func (platform *GLFWPlatform) SetClipboardText(text string) {
